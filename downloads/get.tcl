##
## get file v0.4 by HM2K (based on a script by mamaKIN)
##
### Requirements: ###############################
##
...
set get(nfile) "incoming readme.txt upload.php index.html index.php"

# Which channels shall the commands work on?
set get(chans) "#!serialz #newserialz #serialz"
...
bind msg - [cmdchar]get GET:msg_get
bind pub - [cmdchar]get GET:pub_get
bind pub - [cmdchar]search GET:pub_search
bind pub - [cmdchar]send GET:pub_send
...
proc GET:msg_get {nick host hand text} {
  global get
    ...
    set file $text
    if {![string match "*[string tolower $file]*" "[string tolower [lrange $get(nfile) 0 end]]"]} {
      set results [findfile $file $get(dir)]
    } {
      puthelp "NOTICE $nick :Sorry, file $file is disabled."
      return 1
    }
  ...
  if {[chncheck $chan $get(chans)]} {
    ...
    if {$file == ""} {
	    ...
	    if {[llength $results] == 0} {
	      ...
	      puthelp "NOTICE $nick :File $file was found, to recieve type: /msg $botnick [cmdchar]get $file"
	    }
  }
  return 1
}
  ...
  if {[chncheck $chan $get(chans)]} {
    ...
    if {$file == ""} {
	    ...
	    if {[llength $results] == 0} {
	      ...
	      puthelp "NOTICE $nick :File $results was found as the closest match, to recieve type: /msg $botnick [cmdchar]get $results"
	    }
  }
  return 1
}
...
putlog "Loaded \002get file v0.3\002 by HM2K"
