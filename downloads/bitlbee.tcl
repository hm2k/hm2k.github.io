...
proc dcc:bitlbee {hand idx arg} {
 global nick realname username chanbitlbee bitlbeeidx bitlbeeserv bitlbeenet bitlbeenick shortcommands bitlbeetrim bitlbeechan bitlbeepass
 set bitlbeenick $nick
 ...
 set port [lindex $arg 5]
  ...
  if {$serv == ""} {
   ...
   if {$port == ""} { set bitlbeeidx [connect $serv 6667] } 
   if {$port != ""} { set bitlbeeidx [connect $serv $port] }

   set chanbitlbee $chan
